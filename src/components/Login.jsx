...
//   // ── Google success handler — try login first, auto sign up if not found ───
//   const handleGoogleSignInSuccess = async (credentialResponse) => {
//     setIsProcessingGoogle(true);
//     setErrorMessage("");
...
//       // ── Step 2: Login failed — auto sign up ─────────────────────────────
//       setAuthStatus("creating_account");
...
//       // ── Step 3: Already exists — login again ────────────────────────────
//       if (isUserAlreadyExists(signupStatus, signupData)) {
//         setAuthStatus("signing_in");

//         const { data: retryLoginData, ok: retryLoginOk } =
//           await googleLoginAPI({ email, socialUid, socialToken, imageUrl: googlePicture });

//         if (isLoginSuccess(retryLoginOk, retryLoginData)) {
//           navigateToDashboard(retryLoginData, googlePicture, socialUid, false);
//           return;
//         }
//       }
...
// // Check if user exists by attempting login silently (no loading shown yet)
// const checkUserExistsAPI = async ({ email, socialUid, socialToken, imageUrl }) => {
//   try {
//     const response = await fetch(
//       "https://referralapis.appistan.co/api/auth/login/google",
//       {
...
//       // ── Step 1: Silently check if user exists BEFORE showing loading ────
//       const { data: checkData, ok: checkOk } =
//         await checkUserExistsAPI({ email, socialUid, socialToken, imageUrl: googlePicture });

//       const userExists = isLoginSuccess(checkOk, checkData);

//       // ── Step 2: Now show the correct loading message immediately ────────
//       setAuthStatus(userExists ? "signing_in" : "creating_account");
//       setIsProcessingGoogle(true);

//       // ── Step 3: If user exists, navigate directly with check response ───
//       if (userExists) {
//         navigateToDashboard(checkData, googlePicture, socialUid, false);
//         return;
//       }

//       // ── Step 4: New user — sign up ──────────────────────────────────────
//       const { data: signupData, status: signupStatus, ok: signupOk } =
//         await googleSignUpAPI({ username, email, socialUid, socialToken, imageUrl: googlePicture });
...
//       // ── Step 5: Already exists race condition — login again ─────────────
//       if (isUserAlreadyExists(signupStatus, signupData)) {
//         setAuthStatus("signing_in");
...
//         <div className="bg-white p-8 rounded-lg shadow-lg text-center">
//           <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-[#237D93] mx-auto mb-4"></div>
//           <h2 className="text-xl font-semibold text-[#055860] mb-2">{loadingTitle}</h2>
...
const AuthContent = () => {
  ...
  const handleGoogleSignInSuccess = async (credentialResponse) => {
    ...
    try {
      ...
      // ── Step 1: Silently check if user exists — NO loading shown yet ────
      const { data: checkData, ok: checkOk } =
        await checkUserExistsAPI({ email, socialUid, socialToken, imageUrl: googlePicture });

      const userExists = isLoginSuccess(checkOk, checkData);

      // ── Step 2: NOW show correct loading message based on result ─────────
      // ✅ existing user → "Signing you in..."
      // ✅ new user      → "Creating your account..."
      setAuthStatus(userExists ? "signing_in" : "creating_account");
      setIsProcessingGoogle(true);

      // ── Step 3: Existing user — navigate directly ────────────────────────
      if (userExists) {
        navigateToDashboard(checkData, googlePicture, socialUid, false);
        return;
      }
      ...
      // ── Step 5: Race condition — already exists, login again ─────────────
      if (isUserAlreadyExists(signupStatus, signupData)) {
        setAuthStatus("signing_in");
    ...
    <div className="min-h-screen w-screen bg-[#ECECEC] flex items-center justify-center overflow-hidden">
      <div className="min-h-screen w-full max-w-[4366px] bg-white shadow-lg grid md:grid-cols-2 overflow-hidden">
        ...
        <div className="flex flex-col items-center justify-center py-14 px-8">
          ...
          <div className="w-full max-w-xs space-y-3 mt-5">
            {/* ✅ Button click just triggers Google popup — NO loading here */}
            <button
              onClick={() => {
                googleButtonRef.current?.querySelector('div[role="button"]')?.click();
              }}

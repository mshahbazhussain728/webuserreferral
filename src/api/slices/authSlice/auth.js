...
const authSlice = createSlice({
  ...
  initialState,
  reducers: {
    // ✅ Central setter — always syncs Redux + localStorage
    setUser: (state, action) => {
      state.user       = action.payload;
      state.signedUser = action.payload;
      saveUserToStorage(action.payload);
    ...
    // ✅ OAuth handler — clears old user first, then saves new user with imageUrl
    setOAuthUser: (state, action) => {
      const { accessToken, refreshToken, ...user } = action.payload;
      ...
      // ✅ Clear old user from localStorage first to avoid stale imageUrl
      localStorage.removeItem(USER_KEY);

      // ✅ Save fresh user with correct imageUrl
      state.user       = user;
      state.signedUser = user;
      saveUserToStorage(user);
    },

...
// // FIX 2: Derive the correct mobile page title from the sideBar data.
// // getPageTitles only looks at pathname so all sub-pages of the same route
// // (trial / subscribed / cancelled) return the same title. Instead we also
// // match the ?status= query param so each child gets its own label.
...
//           return child.title; // e.g. "Monthly Trial", "Yearly Subscribed"
//         }
//       }
//     } else {
...
//   // Lifted sidebar open/close state — survives drawer unmount/remount
//   const [openTitle, setOpenTitle] = useState("");

//   // Auto-open the correct parent when URL changes
//   useEffect(() => {
//     const urlStatus = new URLSearchParams(location.search).get("status");
//     const parent = sideBar.find(
//       (item) =>
//         (item.pathname && location.pathname.startsWith(item.pathname)) ||
//         item.inner?.some(
//           (it) =>
//             it.pathname &&
...
//         )
//     );
//     if (parent) setOpenTitle(parent.title);
...
//   // FIX 2: Use our own title resolver instead of getPageTitles for mobile
...
export const Layout = ({ children }) => {
  ...
  // Sync openTitle with the current URL on page load / direct navigation
  useEffect(() => {
    const urlStatus = new URLSearchParams(location.search).get("status");
    const matched = sideBar.find(
      (item) =>
        // Match plain items by pathname
        (!item.inner && item.pathname && location.pathname.startsWith(item.pathname)) ||
        // Match parent items when a child pathname+query matches
        item.inner?.some(
          (it) =>
            it.pathname &&
            location.pathname.startsWith(it.pathname) &&
  ...
  const handleToggle = (item) => {
    if (!item.inner) {
      // Plain item — set openTitle to this item so it goes purple,
      // and clears any open dropdown
  ...
  // No event arg — safe to call directly from inside SideBar links
